...
mod tests {
    use amaru_kernel::{
        CostModels, PREPROD_INITIAL_PROTOCOL_PARAMETERS, ProtocolParameters, TransactionBody, WitnessSet, include_cbor,
        include_json,
    };
    ...
    fn protocol_parameters_with_cost_models(cost_models: CostModels) -> ProtocolParameters {
        let mut pp = PREPROD_INITIAL_PROTOCOL_PARAMETERS.clone();
