...
pub(crate) mod tests {
    ...
    static CONWAY_BLOCK_CONTEXT: LazyLock<AssertPreparationContext> = LazyLock::new(|| AssertPreparationContext {
        utxo: BTreeMap::from([
            (
    ...
    macro_rules! fixture_context {
        ...
        ($hash:literal, $variant:literal) => {
            amaru_kernel::include_json!(concat!("transactions/preprod/", $hash, "/", $variant, "/context.json"))
        };
    }
